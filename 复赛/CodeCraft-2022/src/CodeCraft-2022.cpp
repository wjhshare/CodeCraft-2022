...
vector<BandwithStream*> gAlltimeStreams;
int RunTimes = 0 ;
int loopNumbers = 0;
...
typedef struct Site{
    ...
    vector< unordered_set<BandwithStream*> >mAllocateBandwithStream; // 每个边缘点分配的视频流(vector 是时间t)
...
bool processPer95threshold(vector<string>& mvTime,vector<Site*>& mvSitePtr,vector<Client>& mvClient, double value);
/*****************************************************/
...
#ifdef LOCALPATH
    finish = clock();
    std::cout << "5% 方案运行时间： " << (double)(finish - start) / CLOCKS_PER_SEC << std::endl;
#endif
//    WriteSiteSolution(path.SiteSolutiontxt, mvSite);

    double value = 1.2;
    ret = false;
//    ret = processPer95threshold(mvTime, mvSitePtr, mvClient, value);
...
bool InitialData(vector<string>& mvTime,vector<Site>& mvSite,vector<Client>& mvClient, PathtInf& path){
    ...
    for(int time = 0; time < mvTime.size(); ++time) {
        ...
        for(int s = 0; s < mvSite.size(); ++s){
            ...
            sit->mvLinkStream.push_back(vector<BandwithStream*>());
        }
    ...
    sort(gAlltimeStreams.begin(), gAlltimeStreams.end(), cmpBandwithStream);
...
//            if(AllRunBand < 4*base_cost){   //如果最大的 5% 个时刻的总需求 < 4*base_cost，那么这个边缘节点就不开，即此处会不断跳过，最终这个边缘节点没偷跑
            ...
            for (int st = 0; st < sit->mvLinkStream[time].size(); ++st) {
                BandwithStream *stream = sit->mvLinkStream[time][st];
        ...
        if (sit->mCost > 0 && base_cost > 0) {   //该边缘节点偷跑了，则铺平 0-95% 到 V
            for (int t = 0; t < mvTime.size(); ++t) {
                if (!sit->mIsPer5Time[t]) {
                    vector<BandwithStream *> vecBandwithStream = sit->mvLinkStream[t];
                    for (int st = 0; st < vecBandwithStream.size(); ++st) {
                        BandwithStream *stream = vecBandwithStream[st];
                        if (stream->mIsAllocate) continue;
...
void LQLprocessPer95(vector<string>& mvTime,vector<Site*>& mvSitePtr,vector<Client>& mvClient){
    for(int time = 0; time < mvTime.size(); time++ ){
...
void processPer95(vector<string>& mvTime,vector<Site*>& mvSitePtr,vector<Client>& mvClient){
    // 直接按照gAllStreams中顺序选取最大的
    for(int idx = 0; idx < gAlltimeStreams.size(); idx++ ){
        BandwithStream* stream = gAlltimeStreams[idx];
        if(stream->mIsAllocate) continue;

        int time = stream->mtime;
        Site* sit = chooseSiteToAllocateStream( time, stream );
        if( sit ){
            allocateStreamToSite(time,sit,stream->mCli,stream,0 );
        }else{
...
Site* chooseSiteToAllocateStreamthreshold( int time , BandwithStream* stream, double value){
    Site* ret = NULL;
    double newCostDistance = DBL_MAX;
    Client* cli = stream->mCli;
    for( int idx = 0; idx < cli->mvLinkSite.size(); idx++ ){
        ...
        int threshold = solvethreshold(value,sit->mbandwidth);
        // 选择价值最低的边缘点
        if( threshold  >= siteGetBandwith && !stream->mIsAllocate ){
            if( siteGetBandwith > sit->mCostBand ){
                if(siteGetBandwith < base_cost) //进入该条件只可能是 sit->mCostBand = 0，即还没开的边缘节点（开了的最少都是 base_cost）
                    siteGetBandwith = base_cost;
    ...
    if(ret){
        // 看是否能顶替掉per5的时刻
        int threshold = solvethreshold(value,ret->mbandwidth);
        if(ret->mRealPer5.size() >= 0 ){
            int siteGetBandwith = ret->mvAllocatedBand[time] + stream->mdemand;
            ...
            if( siteGetBandwith > demand ){
                ++loopNumbers;
                int leftDemandBandwith = threshold - ret->mvAllocatedBand[time];
                if( leftDemandBandwith >= stream->mdemand ){
                    allocateStreamToSite(time,ret,stream->mCli,stream,0);
                    leftDemandBandwith -= stream->mdemand;
                }
...
bool processPer95threshold(vector<string>& mvTime,vector<Site*>& mvSitePtr,vector<Client>& mvClient, double value){
    ...
    for(int time = 0; time < mvTime.size(); time++ ){
        ...
        for(int idx = 0; idx < vecBandwithStream.size(); idx++ ){
            ...
            Site* sit = chooseSiteToAllocateStreamthreshold( time, stream, value);
            if( sit ){
                allocateStreamToSite(time,sit,stream->mCli,stream,0 );
            }else{
                ret = false;
